...
`);
// items: global catalog, no day_id
await pool.query(`
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE items SET name = $1, target_qty = $2, unit = $3 WHERE id = $4
       RETURNING id, name, target_qty, unit`,
      [name, target_qty, unit, req.params.id]
    );
...
app.put('/api/claims', async (req, res) => {
  try {
    const { item_id, person_id, day_id } = req.body;
    if (!item_id || !person_id || !day_id) return res.status(400).json({ error: 'item_id, person_id et day_id requis' });
